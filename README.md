...
```bash
npx tsx src/index.ts --input-file <ruta_al_fichero.json>
```

Por ejemplo, usando el fichero de muestra incluido:

```bash
npx tsx src/index.ts --input-file data/sample-precios.json
```
...
node dist/index.js --input-file data/sample-precios.json
```
