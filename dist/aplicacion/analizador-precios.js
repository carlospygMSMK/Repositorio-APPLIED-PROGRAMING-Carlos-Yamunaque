...
export class AnalizadorPrecios {
     ...
     */
    agruparPorDiaSemana(datos, provincia, combustible) {
        const mapa = new Map();
        for (let i = 0; i < 7; i++) {
            mapa.set(i, []);
        }
     ...
     */
    calcularPromedioPorDia(datosPorDia) {
        const diasSemana = [];
        for (let i = 0; i < 7; i++) {
     ...
     */
    calcularPromedioGeneral(diasSemana) {
        if (diasSemana.length === 0) {
            return 0;
        }
     ...
     */
    obtenerPrecioCombustible(estacion, combustible) {
 ...
 */
AnalizadorPrecios.NOMBRES_DIAS = [
    'Domingo',
    'Lunes',
    'Martes',
    'Miércoles',
...
//# sourceMappingURL=analizador-precios.js.map